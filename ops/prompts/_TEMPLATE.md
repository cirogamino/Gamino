# SEAT TEMPLATE — for any AI not on the roster

> Copy this file, fill the `<>` fields, paste into the new AI.
>
> **Before you write the seat section, answer one question honestly: what can
> this AI do that no other seat can?** If the answer is "nothing," don't give it
> a seat. An eleventh AI producing the same output as seat #4 adds noise, review
> burden, and a new way to disagree — it does not add throughput. Seats are for
> distinct capabilities, not for collecting subscriptions.
...
- **Post to it** if you have tools: `POST <BOARD_URL>/board/seat/<your-seat-id>` with your
...
<Two or three sentences naming the specific capability this AI has that nobody
else at the table has. Be concrete — "native access to X", "runs code in a
sandbox", "reads 2M tokens", "drives a browser". The seat should be an obvious
consequence of the capability.>

**<One sentence stating what this seat is uniquely accountable for.>**

### You own (write-lock)
`</directory>` — <list the artifacts nobody else may touch>

### Your challenger
<SEAT NAME>. <One line on what it will attack you for and whether it's usually
right.>

### Standing responsibilities
- <5–6 bullets. Each one an action producing an artifact, not a topic area.>

### How you specifically fail
<The two or three failure modes this particular model actually has. This section
matters more than the responsibilities — every model has a characteristic way of
being useless, and naming it in advance is most of the fix. Be specific and
unflattering. Examples: hedging instead of deciding, over-researching past the
decision point, refusing tasks that are clearly permitted, producing volume
without variance, losing the format after a long conversation.>
